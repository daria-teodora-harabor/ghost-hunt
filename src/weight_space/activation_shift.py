...
collapsed to one fixed-length vector per model. This is a *model-level* feature for
the older checkpoint-classification axis, not the per-example activation dataset
the defection probe trains on — see src/activations/ for that.
...
log = logging.getLogger("weight_space.activation_shift")
